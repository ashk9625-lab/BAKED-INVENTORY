...
export default function WorkManagementClient(){
 const [data,setData]=useState(initialData),[view,setView]=useState('table'),[query,setQuery]=useState(''),[loaded,setLoaded]=useState(false);
 ...
 return <div className="monday-shell">
  ...
  <section className="summary-row">
   <div className="summary-card"><span>Total Items</span><strong>{allItems.length}</strong></div>
   <div className="summary-card"><span>Working</span><strong>{allItems.filter(i=>i.status==='Working on it').length}</strong></div>
   <div className="summary-card"><span>Stuck</span><strong>{allItems.filter(i=>i.status==='Stuck').length}</strong></div>
   <div className="summary-card"><span>Done</span><strong>{allItems.filter(i=>i.status==='Done').length}</strong></div>
  </section>
  ...
  {view==='table'?<div>{groups.map(g=><section className="group-block" key={g.id}>
   <div className="group-head"><input value={g.name} onChange={e=>setData(d=>({...d,groups:d.groups.map(x=>x.id===g.id?{...x,name:e.target.value}:x)}))}/></div>
   <div className="board-table-wrap"><table className="board-table"><thead><tr><th>Item</th><th>Owner</th><th>Status</th><th>Priority</th><th>Due Date</th><th>Notes</th></tr></thead><tbody>
   {g.items.map(i=><tr key={i.id}>
    ...
    <td><select className={`status-${slug(i.status)}`} value={i.status} onChange={e=>updateItem(g.id,i.id,{status:e.target.value})}>{statuses.map(s=><option key={s}>{s}</option>)}</select></td>
    <td><select value={i.priority} onChange={e=>updateItem(g.id,i.id,{priority:e.target.value})}>{priorities.map(p=><option key={p}>{p}</option>)}</select></td>
   ...
   <tr className="add-row"><td colSpan="6"><button onClick={()=>addItem(g.id)}>+ Add Item</button></td></tr>
   </tbody></table></div>
  </section>)}</div>:<div className="kanban-grid">{statuses.map(s=><div className="kanban-col" key={s}>
   <div className={`kanban-title status-${slug(s)}`}><span>{s}</span><strong>{allItems.filter(i=>i.status===s).length}</strong></div>
   {allItems.filter(i=>i.status===s).map(i=><div className="kanban-card" key={i.id}><strong>{i.task}</strong><span>{i.groupName}</span><span>{i.owner}</span><span>{i.priority}</span><select value={i.status} onChange={e=>updateItem(i.groupId,i.id,{status:e.target.value})}>{statuses.map(x=><option key={x}>{x}</option>)}</select></div>)}
  </div>)}</div>}
 </div>
}
